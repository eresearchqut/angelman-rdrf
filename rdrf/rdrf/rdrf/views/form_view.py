...
from rdrf.models.definition.models import CDEFile, RegistryForm, Registry, QuestionnaireResponse
...
class FileUploadView(View):
    ...
    def _entry_exists(self, cd_models, form, section, cde, file_id):
    ...
    def get(self, request, registry_code, file_id):
        ...
        if user.is_patient or user.is_parent or user.is_carrier:
            if user.is_parent:
                ...
                for p in parent.children:
                    patient_registry = p.rdrf_registry.first()
                    if patient_registry:
                        user_registry = patient_registry
                    current_patient_ids.append(p.id)
                if parent.self_patient:
                    current_patient_ids.append(parent.self_patient)
            else:
                user_patient = user.user_object.first()
                current_patient_ids.append(user_patient.id)
                user_registry = user_patient.rdrf_registry.first()
        ...
        if user_registry:
            query_params.update({
                'registry_code': user_registry.code
            })
        if current_patient_ids:
            query_params.update({
                'django_id__in': current_patient_ids
            })
        cd_models = list(ClinicalData.objects.filter(**query_params).order_by('-id').values('django_id', 'data'))
        cdefile = CDEFile.objects.get(pk=file_id)
        patient_id = self._entry_exists(cd_models, cdefile.form_name, cdefile.section_code, cdefile.cde_code, cdefile.id)
